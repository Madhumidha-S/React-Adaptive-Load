...
class SimulationEnvironment:
    ...
    def run_session(self, session_sequence):
        ...
        for i, interaction in enumerate(session_sequence):
            ...
            if isinstance(interaction, dict):
                ...
                dwell_time = interaction.get('duration', interaction.get('dwell_time', 0))
            ...
            if i >= 1:
                ...
                if self.enable_novelties:
                    predictions = self.predictor.predict(recent, registered_ids)
                    self.loader.process_predictions(predictions)
                else:
                    # Baseline: Simple threshold (Guess.js style)
                    # We'll use the bayesian prior as a stand-in for "simpler probability"
                    predictions = self.predictor._predict_bayesian(recent, registered_ids)
                ...
                if i < len(session_sequence) - 1:
                    ...
                    next_actual = next_interaction['component'] if isinstance(next_interaction, dict) else next_interaction
                    
                    # We re-run predict to get top-1 for the next item
                    preds = self.predictor.predict(recent, registered_ids)
                    self.predictor.record_prediction(preds, next_actual)
        ...
        if self.enable_novelties and self.predictor.total_predictions > 20:
            sanitized_seq = [s['component'] if isinstance(s, dict) else s for s in session_sequence]
